...
#let diagram-version = sys.inputs.at("diagram-version", default: "development")
...
#show: paper_notes.with(
  title: "Scientific Diagram Language " + diagram-version,
  subtitle: "Academic renderer visual review",
  authors: ("Pinega Labs / YDMP Research Workspace",),
  paper_id: "pinega-scientific-diagram-language-review",
  stage: "DIAGRAM-REVIEW / ACADEMIC",
...
#note_panel(title: "Review target", kind: "verification", variant: "candidate-strata")[
  Operation intervals are rails rather than UI cards. Linearization points are
  first-class event markers. The time axis, real-time precedence, quiescent
  boundary, and legal sequential witness have distinct visual roles.
]
...
#note_panel(title: "Review target", kind: "verification", variant: "candidate-strata")[
  The stable row head is a reference, not a version object. Versions form the
  dominant temporal chain. Snapshot S17 is shown as a visibility evaluation,
  not as a pointer to the selected version.
]
...
#note_panel(title: "Review target", kind: "verification", variant: "candidate-strata")[
  The lifecycle renderer verifies that the refined typography, edge weights,
  state tones, and print discipline remain coherent across the third shared
  semantic family.
]
